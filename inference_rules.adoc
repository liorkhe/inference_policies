...
* The QDL should serially get the Name from all the Nodes of the SUT, aggregate the response in the Name Method after getting response from all Nodes. Each Node must have a unique Name.   
...
The SUT and QDL must run on two physically separated and distinct systems. SUT can contain multiple Nodes.
